...
class Generator:
    ...
    def __init__(self, args, tokenizer, model, evaluator: Evaluator) -> None:
        ...
        self.enable_answer_checking = args.enable_answer_checking
    ...
    def _get_most_likely_answer_with_self_reward(self, question, responses: List[str], options: dict) -> Tuple[str, float]:
        critiques = []
        for response in responses:
            io_input = self.fewshot_self_reward_config["prompt_template"].format(examples=self.fewshot_self_reward_prompt, instruction=question, response=response)
            io_output_list = self.io.generate(
                io_input,
                num_return=1,
                max_tokens=self.max_tokens,
                stop_tokens=self.fewshot_self_reward_config["stop_tokens"],
            )
            critic = io_output_list[0].strip()
            critiques.append(critic)
        if options is not None:
            most_confident_choices, most_confident_answer_full_completions, _, confidences = self.evaluator.find_most_confident_answer_with_options_and_critiques(
                responses, options, critiques
            )
        else:
            most_confident_choices, most_confident_answer_full_completions, _, confidences = self.evaluator.find_most_confident_answer_with_critiques(
                responses, critiques
            )
        return most_confident_choices, most_confident_answer_full_completions, confidences
    ...
    def _get_most_likely_answer(self, io_output_list: List[str]) -> Tuple[str, float]:
        ...
        if len(io_output_list) == 1:
            ...
            most_confident_choices, most_confident_answer_full_completions, _, confidences = self.evaluator.find_most_confident_answer(
                io_output_list
            )
            # assert confidence > 0
    ...
    def _fewshot_cot_answer_question_using_chat_template(self, question: str, paraphrased: bool, num_return: int, hint: str = None):
        fewshot_cot_prompt = self.fewshot_cot_prompt if not paraphrased else self.fewshot_cot_prompt_rephrased
        question += "\n\n" + hint if hint is not None else ""
        io_input = self.fewshot_cot_config["prompt_template"].format(examples=fewshot_cot_prompt, instruction=question)
        io_output_list = self.io.generate(
    ...
    def _fewshot_cot_rag_answer_question(self, question: str, documents:str, num_return: int, hint: str = None):
        fewshot_cot_rag_prompt = self.fewshot_cot_rag_prompt
        question += "\n\n" + hint if hint is not None else ""
        io_input = self.fewshot_cot_rag_config["prompt_template"].format(examples=fewshot_cot_rag_prompt, instruction=question, documents=documents)
        io_output_list = self.io.generate(
            io_input,
            num_return=num_return,
            max_tokens=self.max_tokens,
    ...
    def _generate_queries_and_search(self, questions):
        query_list, doc_list = [], []
        for question in questions:
            ...
            io_input = (
                    ...
                    + "\n\n"
                    + f"Question {self.question_index}: {question}"
                    + "\n"
                    + f"Query {self.question_index}.{1}:"
            )
            io_output_list = self.io.generate(
                io_input,
                max_tokens=128,
                ...
                stop_tokens=[
                    ...
                    "Document",
                    "Document ",
                    f"Document {self.question_index}.{1}",
                    f"Document {self.question_index}.{1}:",
                    f"Document {self.question_index}.{1}: ",
                ],
            )
            search_query = io_output_list[0].strip()
            document = ""
            if self.retrieval_corpus == "wikipedia":
                request_query = "http://172.16.34.21:6000/api/search?query=" + search_query + "&k=" + str(self.num_retrieval)
            elif self.retrieval_corpus == "textbook":
                request_query = "http://172.16.34.1:6000/api/search?query=" + search_query + "&k=" + str(self.num_retrieval)
            elif self.retrieval_corpus == "textstat":
                request_query = "http://172.16.34.22:6000/api/search?query=" + search_query + "&k=" + str(self.num_retrieval)
    ...
    def _generate_queries_for_user_question(self, user_question):
        decompose_query_prompt = self.decompose_query_prompt
        ...
        for kdx in range(10):
            ...
            query = [o.strip() for o in io_output_list][0]
            if self.retrieval_corpus == "wikipedia":
                request_query = "http://172.16.34.21:6000/api/search?query=" + query + "&k=" + str(self.num_retrieval)
            elif self.retrieval_corpus == "textbook":
                request_query = "http://172.16.34.1:6000/api/search?query=" + query + "&k=" + str(self.num_retrieval)
            elif self.retrieval_corpus == "textstat":
                request_query = "http://172.16.34.22:6000/api/search?query=" + query + "&k=" + str(self.num_retrieval)
            elif self.retrieval_corpus == "medcorp":
                request_query = "http://172.16.34.21:6000/api/search?query=" + query + "&k=" + str(self.num_retrieval)
            x = requests.get(request_query)
            jsobj = json.loads(x.text)
            ...
            for jdx in range(len(jsobj['topk'])):
                ...
                if score >= self.retrieval_threshold:
                    ...
                    scores.append(jsobj['topk'][jdx]['prob'])
            if len(documents) > 0:
                num_queries += 1
                ...
                existing_queries_and_docs += f"Query {self.question_index}." + str(next_query_id) + ": " + query
                existing_queries_and_docs += "\n"
                existing_queries_and_docs += (
                        f"Document {self.question_index}." + str(next_query_id) + ": " + relevant_doc
                )
                existing_queries_and_docs += "\n"
                next_query_id += 1
    ...
    def _search_with_given_queries(self, query_list):
        doc_list = []
        score_list = []
        for query in query_list:
            ...
            document = ""
            scores = []
            for kdx in range(len(jsobj['topk'])):
                score = jsobj['topk'][kdx]['prob']
                if score >= self.retrieval_threshold:
                    document = document + jsobj['topk'][kdx]['text'] + "\n"
                    scores.append(score)
            score_list.append(scores)
            doc_list.append(document)
        return doc_list, score_list

    def _compute_entropy(self, model_values, rag_values):
        model_entropy = 0.0
        rag_entropy = 0.0
        for kdx in range(len(model_values)):
            model_entropy = model_entropy - model_values[kdx] * np.log(model_values[kdx]+ 1e-10)
        for kdx in range(len(rag_values)):
            rag_entropy = rag_entropy - rag_values[kdx] * np.log(rag_values[kdx] + 1e-10)

        total_entropy = model_entropy + rag_entropy

        confidence_normal = (1 - model_entropy / total_entropy)
        confidence_rag = (1 - rag_entropy / total_entropy)

        sum_confidence = confidence_normal + confidence_rag
        weight_normal = confidence_normal / sum_confidence
        weight_rag = confidence_rag / sum_confidence
        return model_entropy, rag_entropy, weight_normal, weight_rag

    def _compute_disagreement(self, model_choices, model_values, rag_choices, rag_values):
        all_choices = {}
        for kdx, choice in enumerate(model_choices):
            all_choices[choice] = {}
            all_choices[choice]["model_value"] = model_values[kdx]
            all_choices[choice]["rag_value"] = 0
        for kdx, choice in enumerate(rag_choices):
            if choice in all_choices.keys():
                all_choices[choice]["rag_value"] = rag_values[kdx]
            else:
                all_choices[choice] = {}
                all_choices[choice]["rag_value"] = rag_values[kdx]
                all_choices[choice]["model_value"] = 0
        kl_1 = 0.0
        kl_2 = 0.0
        for choice in all_choices.keys():
            p_val = all_choices[choice]["model_value"]
            q_val = all_choices[choice]["rag_value"]
            m_val = (p_val+q_val) / 2.0
            kl_1 += p_val * np.log(( p_val + 1e-10) / (m_val + 1e-10))
            kl_2 += q_val * np.log(( q_val + 1e-10) / (m_val + 1e-10))
        jsd = (kl_1 + kl_2) / 2.0
        disagreement = jsd / math.log(2)
        return disagreement

    def _combine_two_choice_distributions(self, model_choices, model_values, model_answers, rag_choices, rag_values, rag_answers):
        if self.enable_majority:
            if len(rag_values) > 0 and rag_values[0] >= self.majority_threshold:
                return rag_choices, rag_values, rag_answers, rag_choices, rag_values, rag_answers
            if len(model_values) > 0 and model_values[0] >= self.majority_threshold:
                return model_choices, model_values, model_answers, model_choices, model_values, model_answers

        if self.combine_distributions == "entropy":
            model_entropy, rag_entropy, confidence_normal, confidence_rag = self._compute_entropy(model_values, rag_values)

        if self.combine_distributions == "disagreement":
            model_entropy, rag_entropy, confidence_normal, confidence_rag  = self._compute_entropy(model_values, rag_values)
            disagreement = self._compute_disagreement(model_choices, model_values, rag_choices, rag_values)
            weight_normal = confidence_normal * (1 + disagreement)
            weight_rag = confidence_rag * (1 - disagreement)
            # import ipdb; ipdb.set_trace()
            # Normalize weights
            total_weight = weight_normal + weight_rag
            weight_normal /= total_weight
            weight_rag /= total_weight
        ...
        top_choice = None
        top_answer = None
        for idx in range(len(model_choices)):
            choice = model_choices[idx]
            all_choices[choice] = {}
            all_choices[choice]["score"] = model_values[idx]
            all_choices[choice]["answer"] = model_answers[idx]
        for idx in range(len(rag_choices)):
            choice = rag_choices[idx]
            if choice in all_choices.keys():
                if self.combine_distributions == "add":
                    score = self.search_query_weight * rag_values[idx] + (1.0 - self.search_query_weight) * all_choices[choice]["score"]
                elif self.combine_distributions == "multi":
                    score = rag_values[idx] * all_choices[choice]["score"]
                elif self.combine_distributions == "subtract":
                    score = rag_values[idx] - all_choices[choice]["score"]
                    if score < 0:
                        score = 0
                elif self.combine_distributions == "entropy":
                    score = confidence_rag * rag_values[idx] + confidence_normal * all_choices[choice]["score"]
                elif self.combine_distributions == "disagreement":
                    score = weight_rag * rag_values[idx] + weight_normal * all_choices[choice]["score"]
                all_choices[choice]["score"] = score
            else:
                all_choices[choice] = {}
                all_choices[choice]["answer"] = rag_answers[idx]
                if self.combine_distributions == "add":
                    all_choices[choice]["score"] = self.search_query_weight * rag_values[idx]
                elif self.combine_distributions == "multi":
                    all_choices[choice]["score"] = 0
                elif self.combine_distributions == "subtract":
                    all_choices[choice]["score"] = rag_values[idx]
                elif self.combine_distributions == "entropy":
                    all_choices[choice]["score"] = confidence_rag * rag_values[idx]
                elif self.combine_distributions == "disagreement":
                    all_choices[choice]["score"] = weight_rag * rag_values[idx]

        for choice in all_choices.keys():
            combined_choices.append(choice)
            combined_answers.append(all_choices[choice]["answer"])
            combined_scores.append(all_choices[choice]["score"])
            sum_score += all_choices[choice]["score"]
        # normalization
        for idx in range(len(combined_scores)):
            combined_scores[idx] = combined_scores[idx] / sum_score
            if combined_scores[idx] > top_score:
                top_score = combined_scores[idx]
                top_choice = combined_choices[idx]
                top_answer = combined_answers[idx]
        # import ipdb; ipdb.set_trace()
        return [top_choice], [top_score], [top_answer], combined_choices, combined_scores, combined_answers

    def _get_top_choice(self, model_choices, model_values, model_answers):
        top_score = -1
        for idx in range(len(model_values)):
            if model_values[idx] > top_score:
                top_score = model_values[idx]
                top_choice = model_choices[idx]
                top_answer = model_answers[idx]
        return [top_choice], [top_score], [top_answer]

    def generate_direct_answers(self, user_question: str, paraphrased: bool, hint: str, options: dict):
    ...
    def generate_direct_answers_with_rag(self, user_question: str, doc_list, hint: str, options: dict):
        direct_answer_list, value_list, choice_list = [], [], []

        # retrieval
        documents = ""
        for idx in range(len(doc_list)):
            documents = documents + doc_list[idx] + "\n"

        # ! few shot cot
        num_return = self.mcts_num_last_votes
        io_input, cleaned_io_output_list = self._fewshot_cot_rag_answer_question(
            question=user_question, documents=documents, num_return=num_return, hint=hint
        )
        try:
            if self.enable_self_reward:
                most_likely_choices, most_likely_answers, likelihoods = self._get_most_likely_answer_with_self_reward(
    ...
    def generate_subquestions(
        ...
        existing_subquestions_and_subanswers, next_subquestion_id, subquestions_history, subanswers_history = concat_subqs_and_subas(
        ...
        for i, cleaned_io_output_group in enumerate(cleaned_io_output_list):
            try:
                if is_final_question:
                    if self.enable_self_reward:
                        choices, most_likely_answers, likelihoods = self._get_most_likely_answer_with_self_reward(user_question, cleaned_io_output_group, options)
                    else:
                        choices, most_likely_answers, likelihoods = self._get_most_likely_answer_with_options(cleaned_io_output_group, options)
                    # choices, most_likely_answers, likelihoods = self._get_most_likely_answer_with_options(cleaned_io_output_group, options)
                else:
            ...
            subanswer_list.extend(most_likely_answers)
            value_list.extend(likelihoods)

        if not is_final_question:
            potential_answers_list = [None] * len(subquestion_list)
            choices = [None] * len(subquestion_list)
            return subquestion_list, subanswer_list, value_list, potential_answers_list, choices
        else:
            if self.enable_answer_checking:
                user_question_context, _ = split_user_question(user_question)
                # query_list, doc_list = self._generate_queries_and_search(subquestions_history)
                # query_list, doc_list = self._generate_queries_for_user_question(user_question)
                doc_list, score_list = self._search_with_given_queries(subquestions_history)
                # question = f"{user_question_context} {subquestion}"
                question = user_question
                documents = "\n".join(doc_list)
                io_input, cleaned_io_output_list = self._fewshot_cot_rag_answer_question(
                    question=question, documents=documents, num_return=self.num_votes
                )
                try:
                    rag_choices, rag_answers, rag_likelihoods = self._get_most_likely_answer_with_options(cleaned_io_output_list, options)
                except Exception as e:
                    raise GeneratorError(
                ...
                if choices is not None and likelihoods is not None and rag_choices is not None and rag_likelihoods is not None:
                    top_choice, top_score, top_answer, combined_choices, combined_values, combined_answers = self._combine_two_choice_distributions(choices, likelihoods, most_likely_answers, rag_choices, rag_likelihoods, rag_answers)
                    potential_answers_list = [None] * len(combined_choices)
                    subquestion_list = [subquestion] * len(combined_choices)
                    return subquestion_list, combined_answers, combined_values, potential_answers_list
                    # potential_answers_list = [None]
                    # subquestion_list = [subquestion]
                    # return subquestion_list, top_answer, top_score, potential_answers_list
                else:
                    return [], [], [], []
            else:
                if choices is not None and likelihoods is not None:
                    # top_choice, top_score, top_answer = self._get_top_answer(choices, likelihoods, most_likely_answers)
                    # potential_answers_list = [None]
                    # return [subquestion], top_answer, top_score, potential_answers_list
                    potential_answers_list = [None] * len(likelihoods)
                    subquestion_list = [subquestion] * len(likelihoods)
                    return subquestion_list, most_likely_answers, likelihoods, potential_answers_list, choices
                else:
                    return [], [], [], [], []
    ...
    def generate_queries(
        ...
        #! generate queries
        existing_queries_and_docs, next_query_id = concat_queries_and_docs(
            solution_trace, self.question_index
        )
        io_input = (
            decompose_query_prompt
        ...
        query_list = [o.strip() for o in io_output_list]
        doc_list = []
        for query in query_list:
            if self.retrieval_corpus == "wikipedia":
                request_query = "http://172.16.34.21:6000/api/search?query=" + query + "&k=" + str(self.num_retrieval)
            ...
            for idx in range(len(jsobj['topk'])):
                document = document + jsobj['topk'][idx]['text'] + "\n"
            doc_list.append(document)
        potential_answers_list = [None] * len(query_list)

        return query_list, doc_list, potential_answers_list
    ...
    def generate_re_subanswers(
        ...
        try:
            choice, most_likely_answer, likelihood = self._get_most_likely_answer(cleaned_io_output_list)
        except Exception as e:
            raise GeneratorError(
                source="generate re-subanswers: few shot cot",
                io_input=io_input,
        ...
        re_subanswer_list.extend(most_likely_answer)
        value_list.extend(likelihood)
    ...
    def generate_re_subanswers_with_rag(
        self,
        user_question: str,
        solution_trace: Dict[int, Dict[str, str]],
        paraphrased: bool,
        ...
        doc_list, score_list = self._search_with_given_queries([last_subquestion])
...
class Reasoning_MCTS_Node(MCTS_Node):
    def __init__(
        ...
        enable_answer_checking: bool=False,
        ...
        if parent is None:  # root
            ...
            self.enable_answer_checking = enable_answer_checking
            ...
            self.enable_answer_checking = parent.enable_answer_checking
    ...
    def _create_children(self):
        def do_action_generate_direct_answers():
            ...
            if (
                ...
                hint = make_hint(self.solution_trace, False)
            else:
                hint = None

            (direct_answer_list, value_list, choice_list) = self.generator.generate_direct_answers(
                user_question=self.user_question, paraphrased=self.paraphrased, hint=hint, options=self.options
            )
            if self.enable_answer_checking:
                query_list, doc_list, score_list, document_list = self.generator._generate_queries_for_user_question(self.user_question)
                # query_list, doc_list = self.query_list, self.document_list
                rag_hint = make_hint(self.solution_trace, True, query_list, doc_list)
                if hint is not None:
                    rag_hint = rag_hint + "\n"+ hint
                # (rag_answer_list, rag_value_list, rag_choice_list) = self.generator.generate_direct_answers_with_rag(
                #     user_question=self.user_question, doc_list=doc_list, hint=None, options=self.options
                # )
                (rag_answer_list, rag_value_list, rag_choice_list) = self.generator.generate_direct_answers(
                    user_question=self.user_question, paraphrased=self.paraphrased , hint=rag_hint, options=self.options
                )
                top_choice, top_score, top_answer, combined_choices, combined_values, combined_answers = self.generator._combine_two_choice_distributions(choice_list, value_list, direct_answer_list, rag_choice_list, rag_value_list, rag_answer_list)
                direct_answer_list, value_list = combined_answers, combined_values
                # direct_answer_list, value_list = top_answer, top_score
            # else:
            #     top_choice, top_score, top_answer = self.generator._get_top_choice(choice_list, value_list, direct_answer_list)
            #     import ipdb; ipdb.set_trace()
            #     direct_answer_list, value_list = top_answer, top_score
        ...
        def do_action_generate_direct_answers_with_rag():
            verbose_print(f"---- Generating direct answers with RAG for node {self.id}...", self.verbose)
            ...
            query_list, doc_list, score_list, document_list = self.generator._generate_multiple_queries_for_user_question(
                self.user_question)
            # query_list, doc_list = self.query_list, self.document_list
            # import ipdb; ipdb.set_trace()
            rag_hint = make_hint(self.solution_trace, True, query_list, doc_list)
            ...
            for direct_answer, value, choice in zip(rag_answer_list, rag_value_list, rag_choice_list):
                ...
                self.children.append(
                    Reasoning_MCTS_Node(
                        parent=self,
                        depth=self.depth + 1,
                        node_type=Node_Type.DIRECT_ANSWER_RAG,
                        node_value=value,
                        direct_answer=direct_answer,
                        choice=choice
                    )
        ...
        def do_action_generate_subquestions():
            ...
            (subquestion_list, subanswer_list, value_list, potential_answers_list, choice_list) = (
                self.generator.generate_subquestions(
                    user_question=self.user_question, solution_trace=self.solution_trace, paraphrased=self.paraphrased, options=self.options
                )
            )
            for subquestion, subanswer, value, potential_answers, choice in zip(
                subquestion_list, subanswer_list, value_list, potential_answers_list, choice_list
            ):
                if np.isnan(value) or value <= 0:
                    continue
        ...
        def do_action_generate_queries():
            verbose_print(f"---- Generating queries for node {self.id}...", self.verbose)

            #! ACTION: generate new subquestions
            (query_list, doc_list, potential_answers_list) = (
                self.generator.generate_queries(
            ...
            #     import ipdb; ipdb.set_trace()
            for query, document, potential_answers in zip(
                query_list, doc_list, potential_answers_list
            ):
                self.children.append(
                    Reasoning_MCTS_Node(
                        parent=self,
                        depth=self.depth + 1,
                        node_type=Node_Type.SEARCH_QUERY,
                        query=query,
                        document=document,
                        is_new_query=True,
                        potential_answers=deepcopy(potential_answers),
        ...
        def do_action_generate_re_subanswers_rag():
            verbose_print(f"---- Generating re-subanswers with RAG for node {self.id}...", self.verbose)

            #! ACTION: re-generate subanswers for the previous subquestion
            (re_subanswer_list, value_list, potential_answers_list) = self.generator.generate_re_subanswers_with_rag(
                user_question=self.user_question,
                solution_trace=self.solution_trace,
                paraphrased=self.paraphrased,
                options=self.options
            ...
            for re_subanswer, value, potential_answers in zip(re_subanswer_list, value_list, potential_answers_list):
                ...
                self.children.append(
                    Reasoning_MCTS_Node(
                        parent=self,
                        depth=self.depth + 1,
                        node_type=Node_Type.RE_SUBANSWER_RAG,
                        node_value=value,
                        re_subanswer=re_subanswer,
                        potential_answers=deepcopy(potential_answers),
                    )
        ...
        if self.node_type is Node_Type.USER_QUESTION:
            ...
            # A6: direct answer with rag
            if not self.disable_a6:
                do_action_generate_direct_answers_with_rag()

            # A5: Rephrase the question/sub-question.
            if not self.disable_a5:
                do_action_generate_rephrased_user_question()

            # A8: generate a search query then retrieve
            if not self.disable_a8:
                do_action_generate_queries()
            ...
            # A7: Answer the sub-question again with RAG.
            if not self.disable_a7:
                do_action_generate_re_subanswers_rag()

        elif self.node_type is Node_Type.RE_SUBANSWER or self.node_type is Node_Type.RE_SUBANSWER_RAG:
        ...
        elif self.node_type is Node_Type.SEARCH_QUERY:
            ...
            # A6: generate a search query then retrieve
            if not self.disable_a6:
                do_action_generate_queries()
    ...
    def is_valid_leaf_node(self):
        ...
        ) or self.node_type is Node_Type.DIRECT_ANSWER or self.node_type is Node_Type.DIRECT_ANSWER_RAG
    ...
    def get_choice(self):
        ...
        valid, choice = self.generator.evaluator.check_valid_answer(answer, completion, self.options, 60)
        if valid:
            return choice
        return None


def search_for_answers(args, user_question: str, question_id: int, gt_answer: str, generator: Generator, options: dict):
    verbose_print(
