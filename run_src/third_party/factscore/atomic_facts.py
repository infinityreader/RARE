...
class AtomicFactGenerator(object):
  ...
  def __init__(
      ...
      other_lm: Optional[open_modeling.Model] = None,
  ):
  ...
  def run(self, generation: str, cost_estimate: Optional[bool] = None, split_fact=False):
    ...
    return self.get_atomic_facts_from_paragraph(
        paragraphs, cost_estimate=cost_estimate, split_fact=split_fact
    )

  def get_atomic_facts_from_paragraph(self, paragraphs, cost_estimate=None, split_fact=False):
    ...
    if not split_fact:
        ...
        for sentence in sentences:
            atomic_facts_pairs.append((sentence, [sentence]))
        return atomic_facts_pairs, [1]
